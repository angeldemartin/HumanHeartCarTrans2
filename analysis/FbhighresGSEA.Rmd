...
```{r glycoprotein metabolic process}
ego1 <- dplyr::filter(ego@result, ego@result$Description=="glycoprotein metabolic process")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
```{r cellular response to peptide hormone stimulus}
ego1 <- dplyr::filter(ego@result, ego@result$Description=="cellular response to peptide hormone stimulus")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
```{r GSEA Fb4, fig.height=12, fig.width=12}
## adjust table
markerG <- dplyr::filter(markerGenes, cluster == "Fb4")
markerG <- markerG %>%
  mutate(Gene=gsub("^.*\\.", "", gene))  %>%
  mutate(EnsID=gsub("\\..*","", gene))
...
### extracellular matrix organization
```{r extracellular matrix organization Fb4}
ego1 <- dplyr::filter(ego@result, ego@result$Description=="extracellular matrix organization")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
...
```{r muscle tissue development}
ego1 <- dplyr::filter(ego@result, ego@result$Description=="muscle tissue development")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
...
```{r nucleotide-sugar biosynthetic process}
ego1 <- dplyr::filter(ego@result, ego@result$Description=="nucleotide-sugar biosynthetic process")
g1 <- ego1$geneID
Str <-(g1)
StrSub <- strsplit(Str, "/")
df <- as.data.frame(StrSub)
