...
```{r}
assosRes <- associationTest(sceGAM)
head(assosRes)
```

```{r}
## comparing start and end point
startRes <- startVsEndTest(sceGAM)
oStart <- order(startRes$waldStat, decreasing = TRUE)
head(rownames(startR)[oStart])
```
...
rankGene <- rownames(startRes)[oStart]
...
```{r}
## comparing selected pseutotime values
startRes <- startVsEndTest(sceGAM, pseudotimeValues = c(0.1, 10))
oStart <- order(startRes$waldStat, decreasing = TRUE)
head(rownames(startR)[oStart])
```
...
lapply(selGene,function(selGene){
  plotGeneCount(sceGAM, counts(sceGAM), gene = selGene, curvesCols=colLin) +
    ggtitle(selGene) +
    scale_color_manual(values=colLin)
})
```
